...
如果没有找到合适的匹配，返回：
{
  ...
  static resolveSourceOrder() {
    let sourceOrder = this.get('SOURCE_ORDER', 'douban,dandan,360,renren,hanjutv', 'string');
    ...
    return orderArr.length > 0 ? orderArr : ['douban', 'dandan', '360', 'renren', 'hanjutv'];
  }
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      // 源配置
      'SOURCE_ORDER': { category: 'source', type: 'multi-select', options: this.ALLOWED_SOURCES, description: '源排序配置，默认douban,dandan,360,renren,hanjutv' },
